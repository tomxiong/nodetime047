...
exports.init = function(_nt) {
  ...
  nt.timers.push(setInterval(function() {
    try {
      exports.sendInfo();
      collectMetrics();
    }
...
exports.sendInfo = function() {
  ...
  try { info['Total memory'] = os.totalmem() /1024 } catch(err) { nt.error(err) } 
  ...
  try { info['OS uptime'] = Math.floor(os.uptime()) } catch(err) { nt.error(err) } 
  try { info['Node arguments'] = process.argv } catch(err) { nt.error(err) } 
  try { info['Node PID'] = process.pid; } catch(err) { nt.error(err) } 
  try { info['Node uptime'] = Math.floor(process.uptime()); } catch(err) { nt.error(err) } 
...
var collectMetrics = function() {
  ...
  //try { nt.metric('OS', 'Free memory', os.freemem() / 1000, 'KB'); } catch(err) { nt.error(err); }


  try {
    var mem = process.memoryUsage();
    nt.metric('Process', 'Node RSS', mem.rss / 1024, 'KB');
    nt.metric('Process', 'V8 heap used', mem.heapUsed / 1024, 'KB');
    nt.metric('Process', 'V8 heap total', mem.heapTotal / 1024, 'KB');
  }
  catch(err) {
    nt.error(err);
  }
  ...
  var uvmons = v8profiler.getUVData();
  nt.metric('Process', 'UV count', uvmons.count,  1, undefined, 'sum');
  nt.metric('Process', 'UV sum', uvmons.sum_ms, 'ms');
  nt.metric('Process', 'UV slowest', uvmons.slowest_ms, 'ms');
};
