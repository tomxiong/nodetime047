...
var Nodetime = function() {
  ...
  // MR: keeping nodetime's timers on the side so we can clean them up after
  this.timers = [];
...
var NodetimeExposed = function() {
  ...
  ['on', 'addListener'].forEach(function(meth) {
    self[meth] = function() { 
      // MR: we want metrics and info as well
  ...
  // MR: some internals have to be exposed for our use
  this.tools = function() { return nodetime.tools };
  this.timers = nodetime.timers;
};
